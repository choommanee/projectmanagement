...
// GET  /api/documents/{id}/sign-envelopes  → { items, total }
// POST /api/documents/{id}/sign-envelopes  → 201 envelope
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/documents/${id}/sign-envelopes${url.search}`, { headers: h });
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/documents/${id}/sign-envelopes`, { method: "POST", headers: h, body });
  return new NextResponse(await r.text(), { status: r.status, headers: { "content-type": "application/json" } });
}
