...
// Worklog edit/delete on project-svc (:8083) at /v1/worklogs/{id}.
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/worklogs/${id}`, { method: "PATCH", headers: h, body });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/worklogs/${id}?${url.searchParams.toString()}`, { method: "DELETE", headers: h });
