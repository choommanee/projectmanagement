...
export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/templates/${id}`, { headers: h });
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SVC}/v1/templates/${id}`, { method: "PATCH", headers: h, body });
  return new NextResponse(await r.text(), { status: r.status, headers: { "content-type": "application/json" } });
}
...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const h = await makeHeaders();
  if (h instanceof NextResponse) return h;
  const r = await fetch(`${SVC}/v1/templates/${id}`, { method: "DELETE", headers: h });
  const text = await r.text();
  return new NextResponse(text || null, { status: r.status, headers: { "content-type": "application/json" } });
}
