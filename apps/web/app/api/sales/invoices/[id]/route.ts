...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const h = await makeHeaders();
  if (h instanceof NextResponse) return h;
  // Backend has no hard-delete for invoices; cancel by setting status=cancelled
  const gr = await fetch(`${SALES_URL}/v1/invoices/${id}`, { headers: h });
  if (!gr.ok) return new NextResponse(await gr.text(), { status: gr.status });
  const inv = await gr.json() as Record<string, unknown>;
  const version = inv.Version ?? inv.version ?? 1;
  h.set("content-type", "application/json");
  const r = await fetch(`${SALES_URL}/v1/invoices/${id}`, {
