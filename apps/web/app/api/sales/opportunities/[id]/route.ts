...
export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SALES_URL}/v1/opportunities/${id}`, { headers: h });
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SALES_URL}/v1/opportunities/${id}`, { method: "PATCH", headers: h, body });
  return new NextResponse(await r.text(), { status: r.status, headers: { "content-type": "application/json" } });
}
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await fetch(`${SALES_URL}/v1/opportunities/${id}${qs}`, { method: "DELETE", headers: h });
