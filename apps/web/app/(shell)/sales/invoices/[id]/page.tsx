...
export default function SalesInvoiceDetailPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-line bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-ink-3 mt-1">{customer?.name ?? inv.customer_id}</p>
            {isOverdue && (
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {(inv.status === "draft" || inv.status === "sent") && (
            <button onClick={() => handleStatus("cancelled")} disabled={saving}
              className="px-4 py-1.5 text-sm border border-line rounded hover:bg-surface-2 disabled:opacity-50">
              Cancel
            </button>
          )}
        </div>
      ...
      {/* Details grid */}
      <div className="rounded-lg border border-line bg-card p-6">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-ink-3 mb-4">Details</h2>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
          <div><dt className="text-ink-3">Invoice #</dt><dd className="font-medium">{inv.code ?? inv.id}</dd></div>
          <div><dt className="text-ink-3">Customer</dt><dd className="font-medium">{customer?.name ?? inv.customer_id}</dd></div>
          <div><dt className="text-ink-3">Issue Date</dt><dd className="font-medium">{inv.issue_date}</dd></div>
          <div><dt className="text-ink-3">Due Date</dt><dd className="font-medium">{inv.due_date ?? "—"}</dd></div>
          <div><dt className="text-ink-3">SO Reference</dt><dd className="font-medium">{inv.so_id ?? "—"}</dd></div>
          <div><dt className="text-ink-3">Notes</dt><dd className="font-medium">{inv.notes ?? "—"}</dd></div>
        </dl>
      </div>
    </div>
  );
