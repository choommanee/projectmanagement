  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <div className="px-4 py-3 bg-muted/50 text-sm font-medium">Journal Lines ({je.lines.length})</div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {je.lines.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">No lines</td></tr>}
            {je.lines.map(l => (
              <tr key={l.id} className="border-t border-border hover:bg-muted/20">
                ...
                <td className="px-4 py-3 text-xs text-muted-foreground">{l.memo}</td>
