  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-ink">SA Workspace</h1>
          <p className="mt-1 text-sm text-ink-3">SDD / ADR / ER Diagrams / API Spec — pick a project to open its workspace.</p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((p) => (
              <button
                type="button"
                key={p.id}
                onClick={() => router.push(`/pm/sa/${p.id}`)}
