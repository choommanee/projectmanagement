    ...
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-line bg-surface p-5 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs text-ink-3 font-mono">{insp.inspectedAt?.slice(0, 10)}</span>
          </div>
      ...
      {insp.measurements && insp.measurements.length > 0 && (
        <div className="rounded-lg border border-line overflow-hidden">
          <div className="px-4 py-3 bg-surface-2 text-sm font-medium">Measurements ({insp.measurements.length})</div>
      ...
      {(!insp.measurements || insp.measurements.length === 0) && (
        <div className="rounded-lg border border-line px-4 py-6 text-center text-sm text-ink-3">No measurements recorded</div>
      )}
    </div>
  );
}
