          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {fmeas.map(f => {
                ...
                return (
                  <tr key={f.id} onClick={() => router.push(`/mfg/fmea/${f.id}`)}
              ...
              {!loading && fmeas.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-ink-3">No FMEA records found.</td></tr>
              )}
            </tbody>
          </table>
        )}
