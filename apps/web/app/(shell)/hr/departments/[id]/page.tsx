      ...
      <div className="rounded-lg border border-line bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-ink-3 mt-1">Code: {dept.code}</p>
          </div>
          <span className={`px-2 py-1 rounded text-xs font-medium ${dept.active ? 'bg-green-100 text-green-800' : 'bg-muted text-ink-3'}`}>
            ...
            <tbody>
              {employees.map(emp => (
                <tr key={emp.id} className="border-t border-line hover:bg-surface-2 cursor-pointer"
                  ...
                  <td className="py-2 pr-4 text-ink-3">{emp.positionName ?? "—"}</td>
                  <td className="py-2">
