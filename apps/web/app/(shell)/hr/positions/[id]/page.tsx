    ...
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className={`px-2 py-1 rounded text-xs font-medium ${pos.active ? 'bg-green-100 text-green-800' : 'bg-muted text-muted-foreground'}`}>
            ...
            <tbody>
              {employees.map(emp => (
                <tr key={emp.id} className="border-t border-border hover:bg-muted/30 cursor-pointer"
                  ...
                  <td className="py-2 pr-4 text-muted-foreground">{emp.departmentName ?? "—"}</td>
                  <td className="py-2">
