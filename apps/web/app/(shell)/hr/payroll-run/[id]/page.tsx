      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{run.period_start} – {run.period_end}</p>
          </div>
      ...
      {payslips.length > 0 && (
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {payslips.map(ps => (
                <tr key={ps.id} className="border-t border-border hover:bg-muted/30 cursor-pointer"
                  ...
                  <td className="py-2">
                    <span className={`px-2 py-0.5 rounded text-xs ${ps.status === 'paid' ? 'bg-green-100 text-green-800' : ps.status === 'approved' ? 'bg-blue-100 text-blue-800' : 'bg-muted text-muted-foreground'}`}>
                      {ps.status}
                    </span>
                  </td>
                </tr>
