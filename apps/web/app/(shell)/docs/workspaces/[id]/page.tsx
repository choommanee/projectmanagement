      ...
      <div className="rounded-lg border border-line bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-ink-3 mt-1">Project: {ws.projectId}</p>
          </div>
          <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${kindColors[ws.kind] ?? "bg-muted text-ink-3"}`}>
            {ws.kind}
          </span>
        </div>
      </div>
      ...
      {docs.length > 0 && (
        <div className="rounded-lg border border-line bg-card p-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {docs.map(doc => (
                <tr key={doc.id} className="border-t border-line hover:bg-surface-2 cursor-pointer"
                  onClick={() => router.push('/docs/documents/' + doc.id)}>
                  <td className="py-2 pr-4 font-medium">{doc.title}</td>
                  <td className="py-2 pr-4 text-ink-3 capitalize">{doc.type?.replace("_", " ") ?? "—"}</td>
                  <td className="py-2 pr-4">
                    <span className={`px-2 py-0.5 rounded text-xs ${statusColors[doc.status] ?? ""}`}>
                      {doc.status}
                    </span>
                  </td>
                  <td className="py-2 text-ink-3">{doc.updatedAt ? doc.updatedAt.slice(0, 10) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
