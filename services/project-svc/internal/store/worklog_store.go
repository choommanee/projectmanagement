...
func (s *WorklogStore) List(ctx context.Context, tenantID, taskID uuid.UUID) ([]domain.WorklogEntry, error) {
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		rows, err := tx.Query(ctx,
			`SELECT id, tenant_id, task_id, user_id, logged_md, work_date, note, version, created_at, updated_at
			 FROM task_worklog WHERE task_id = $1 AND deleted_at IS NULL ORDER BY work_date DESC, created_at DESC`,
			taskID)
		if err != nil {
			return err
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&e.ID, &e.TenantID, &e.TaskID, &e.UserID,
				&e.LoggedMd, &e.WorkDate, &e.Note, &e.Version, &e.CreatedAt, &e.UpdatedAt); err != nil {
				return err
			}
			out = append(out, e)
		}
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`INSERT INTO task_worklog (tenant_id, task_id, user_id, logged_md, work_date, note)
			 VALUES ($1,$2,$3,$4,$5,$6)
			 RETURNING id, tenant_id, task_id, user_id, logged_md, work_date, note, version, created_at, updated_at`,
			tenantID, p.TaskID, p.UserID, p.LoggedMd, p.WorkDate, p.Note,
		).Scan(&e.ID, &e.TenantID, &e.TaskID, &e.UserID, &e.LoggedMd, &e.WorkDate, &e.Note, &e.Version, &e.CreatedAt, &e.UpdatedAt)
	})
	return e, err
}

// Update edits a worklog entry with optimistic locking and returns the
// updated record. Nil fields in p are left unchanged.
func (s *WorklogStore) Update(ctx context.Context, tenantID uuid.UUID, p domain.UpdateWorklogParams) (domain.WorklogEntry, error) {
	var e domain.WorklogEntry
	loggedArg := func() interface{} {
		if p.LoggedMd == nil {
			return nil
		}
		return *p.LoggedMd
	}
	workDateArg := func() interface{} {
		if p.WorkDate == nil {
			return nil
		}
		return *p.WorkDate
	}
	noteArg := func() interface{} {
		if p.Note == nil {
			return nil
		}
		return *p.Note
	...
	err := s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`UPDATE task_worklog SET
			   logged_md = COALESCE($3, logged_md),
			 ...
			 RETURNING id, tenant_id, task_id, user_id, logged_md, work_date, note, version, created_at, updated_at`,
			p.ID, tenantID, loggedArg(), workDateArg(), noteArg(), p.Version,
		).Scan(&e.ID, &e.TenantID, &e.TaskID, &e.UserID, &e.LoggedMd, &e.WorkDate, &e.Note, &e.Version, &e.CreatedAt, &e.UpdatedAt)
	})
...
// Delete soft-deletes a worklog entry by ID using optimistic locking.
func (s *WorklogStore) Delete(ctx context.Context, tenantID, id uuid.UUID, version int) error {
	return s.withTenant(ctx, tenantID, func(tx pgx.Tx) error {
		ct, err := tx.Exec(ctx,
			`UPDATE task_worklog SET deleted_at = now(), updated_at = now(), version = version + 1
