...
// auditWrite records a successful mutation in the platform audit_log. The actor
// is taken from the verified JWT claims in context; tenant from the resolved
// X-Tenant-Id. Best-effort: never blocks or fails the request.
...
// clientIP returns a clean IP (no port) suitable for the audit_log.ip ::inet
// column, or "" when none can be parsed.
