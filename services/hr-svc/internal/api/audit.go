...
// the verified JWT claims; tenant falls back to the X-Tenant-Id header so the
// row is always tenant-scoped.
...
// clientIP extracts a bare IP suitable for the inet column. Returns "" when
// none can be determined (the publisher then writes NULL).
