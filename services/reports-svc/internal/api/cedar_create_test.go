...
func seedCedarTenant(t *testing.T, p *pgxpool.Pool) uuid.UUID {
	...
	_, err := p.Exec(context.Background(),
		...
		tid, "cedar-reports-"+tid.String()[:8], "Cedar Reports Test Tenant")
	if err != nil {
...
func TestCedarGatesDashboardCreate_Denies403WithoutAllowedRole(t *testing.T) {
	p := cedarTestPool(t)
	defer p.Close()
	tid := seedCedarTenant(t, p)
	...
	router := newCedarHandler(p, authz)
	// mfg-operator has no permit for report.dashboard.create in the shared bundle.
	h := withClaims(router, &libauth.ParsedClaims{
